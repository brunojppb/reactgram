...
export const AddPostPage = () => {
  const maxTitle = 144
  const [{ pictureId, pictureUrl, title, isUploading }, set] = useState(
    defaultState
  )
  ...
  const counterColor =
    charsLeft >= 80 ? null : charsLeft <= 40 ? 'orangered' : 'orange'
  ...
  const onSave = () => {
    ...
    if (title.length < 3) {
      showNotification(
        'You need to write a litle. Cmon, a picture without text is boring.'
      )
      return
    }
    if (pictureId === null) {
      showNotification(
        'A post without a picture is sad😞 Please, upload a picture.'
      )
    }

    set((state) => ({ ...state, isUploading: true }))
    postCreatePost(pictureId, title).then(
      (response) => {
  ...
  return (
    <div className="add-post-container">
      ...
      <button
        className="btn btn-primary save-post"
        onClick={onSave}
        disabled={isUploading}
      >
        Send
